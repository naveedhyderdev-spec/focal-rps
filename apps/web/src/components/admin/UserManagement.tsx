...
export function UserManagement() {
  ...
  const rows = useMemo(() => [...(users ?? [])].sort((a, b) => ALL_ROLES.indexOf(a.role) - ALL_ROLES.indexOf(b.role) || a.name.localeCompare(b.name)), [users]);
  ...
  return (
    <div className="card">
      <div className="card-header">
        <div><div className="card-title">User Management</div><div className="muted" style={{ fontSize: 'var(--text-xs)', marginTop: 2 }}>Appoint or remove Admins, link Staff to their resource. Master-Admin only.</div></div>
        <button className="btn btn-primary btn-sm" onClick={openAdd}><i className="ti ti-user-plus" /> Invite user</button>
      </div>
      <div className="table-wrap">
        <table className="table">
          <thead><tr><th>Name</th><th>Email</th><th>Role</th><th>Linked resource</th><th>Status</th><th style={{ width: 220 }}>Actions</th></tr></thead>
          <tbody>
            {rows.map((u) => (
              <tr key={u.id}>
                ...
                <td>{u.resource_id ? (resourceById.get(u.resource_id)?.forename ?? '—') : <span className="muted">—</span>}</td>
      ...
      <ConfirmDialog open={!!toDelete} title="Remove user" message={`Remove ${toDelete?.name}'s account? This cannot be undone.`} onConfirm={confirmDelete} onCancel={() => setToDelete(null)} busy={remove.isPending} />
    </div>
  );
}
