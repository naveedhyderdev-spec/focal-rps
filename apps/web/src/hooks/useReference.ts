...
import type { Discipline, Grade, Team, Location, StageType, ProjectTypeOption } from '@engine';
import { disciplinesH, gradesH, teamsH, locationsH, stageTypesH, projectTypesH } from './useData';
...
export function useReference() {
  ...
  const projectTypes = projectTypesH.useList().data ?? [];
  ...
  return useMemo(() => {
    ...
    return {
      ...
      projectTypeById: byId(projectTypes) as Map<string, ProjectTypeOption>,
    };
  }, [disciplines, grades, teams, locations, stageTypes, projectTypes]);
}
